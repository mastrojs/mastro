    ...
    <html lang="en">
      <head>
        ...
        try {
          ...
          const { loadRoutes } = await import("@mastrojs/mastro/server-filebased")
...
const getImportMap = async (
  ...
  const readImports = async (importMapPath: string): Promise<object> => {
    try {
      ...
      return JSON.parse(decoder.decode(bs))?.imports || {};
    } catch (e) {
  ...
  if (false && vscode.ExtensionMode.Development === context.extensionMode) {
    ...
    const mastroProdUrl = "https://esm.sh/jsr/@mastrojs/mastro@0.8.4/";
