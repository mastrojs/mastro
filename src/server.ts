...
export const createHandler = (opts?: CreateHandlerOpts): Handler => async (req: Request) => {
  ...
  try {
    ...
    if (method === "GET" && opts?.serveStaticFiles) {
    ...
    const modPath = "./routers/fileRouter.ts"; // in variable to prevent bundling by esbuild
    const routes: Route[] = opts?.routes ||
      await import(modPath).then(({ getRoutes }) => getRoutes());
