...
import type { TodoPost } from "./todo/index.server.ts";
...
globalThis.customElements?.define(
  ...
  class extends ReactiveElement {
    ...
    async toggleTodo(i: number, e: Event) {
      ...
      const res = await fetchApi<TodoPatch>("PATCH", `/todo/${todo.id}`, {
        done: todo.done,
      });
      if (!res.ok) {
    ...
    async addTodo(e: Event) {
      ...
      if (title) {
        ...
        const res = await fetchApi<TodoPost>("POST", "/todo/", newTodo);
        if (!res.ok) {
          // if saving to server fails, rollback GUI
